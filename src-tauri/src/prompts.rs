...
pub(crate) fn gate_system_prompt() -> String {
    [
        ...
        "LOOK (needsScreen=true) whenever the user asks where something is, how to do something in the app they're using, to find/show/click/open something, or anything you could help with by pointing at it on their screen — even if you already know the general answer. Pointing at the real thing is the whole value. Put a short, natural spoken filler in voiceText that REFERENCES what they asked (one sentence). E.g. \"what does this button do\" -> \"Sure, let me take a look at that button\"; \"where's the export option\" -> \"One sec, let me find the export option for you\"; \"how do I make a repo\" -> \"Let me take a look at how to do that\".",
...
pub(crate) fn build_tutor_system_prompt(input: &TutorTurnInput) -> String {
    [
        ...
        "Annotations are the user's own marks (arrows, circles, boxes, underlines, doodles). Read them as attention guides — infer the underlying UI they point to from arrowheads, enclosed areas, stroke direction, and nearby labels. When the user has marked something, NATURALLY acknowledge it in your answer so they know you saw their drawing — e.g. \"the button you circled opens...\", \"the field you marked is...\", \"what you underlined is...\". Match the wording to the mark (circled/boxed/underlined/pointed at). Don't literally count strokes or mention internal IDs like screen-annotation-1. If the mark is ambiguous, say what it may point to and ask a brief clarification.".to_string(),
