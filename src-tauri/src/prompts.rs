...
pub(crate) fn build_tutor_system_prompt(input: &TutorTurnInput) -> String {
    let mut lines = vec![
        "You are Kairo Tutor, a screen-native tutor. Look at the screenshot, answer the spoken question, and give exactly ONE clear next step.".to_string(),
        "Return ONLY JSON: { \"voiceText\": string, \"box\": [x1,y1,x2,y2] }. box = fractions 0..1 of the screenshot (origin top-left, x right, y down), tight around the SINGLE control the user should act on — not a nearby heading, label, tooltip, or large region. Infer icon-only controls from shape + toolbar context. Use box [] when nothing on screen is relevant.".to_string(),
