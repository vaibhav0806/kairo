...
pub(crate) fn build_tutor_system_prompt(input: &TutorTurnInput) -> String {
    [
        ...
        "VisualTarget = { kind, label, elementId?, screenRegion? }. kind: pointer (click point), highlight_box (control/region), arrow (drag/direction), ghost_cursor, underline (text/field row), spotlight (area). For visible text set elementId from SCREEN ELEMENTS; for icons, buttons, or objects return a tight screenRegion in display points using screen.displayBounds. Infer icon-only tools from shape + toolbar context.".to_string(),
        "For where/how/show questions, point at the exact control (prefer highlight_box + pointer). Use at most 3 targets, best first; return [] if nothing on screen is relevant. Ignore Kairo's own notch, answer card, purple labels, cursor, and overlays unless the user asks about Kairo.".to_string(),
