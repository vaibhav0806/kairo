...
pub(crate) fn build_tutor_system_prompt(input: &TutorTurnInput) -> String {
    let mut lines = vec![
        ...
        "Return ONLY JSON: { \"steps\": [ { \"say\": string, \"box\": [x1,y1,x2,y2] | null } ], \"await_click\": { \"box\": [x1,y1,x2,y2], \"wait\": \"instant\"|\"ui-settle\"|\"page-load\"|\"network\" } | null, \"done\": boolean }.".to_string(),
        "For a HANDS-ON action the user should perform, put the instruction in a step's \"say\" (box usually null) and put the SINGLE thing to click in \"await_click\". Give ONE actionable step at a time — never dump the whole task. \"await_click.wait\" = how long the screen takes to settle AFTER that click: \"instant\" (focus/toggle), \"ui-settle\" (menu/panel opens), \"page-load\" (open file / switch tab), \"network\" (submit / merge / server round-trip).".to_string(),
        "For orientation or explanation where nothing needs clicking, use \"steps[].box\" highlights and \"await_click\": null. Use 1 step for a simple, direct answer; several to orient the user on an unfamiliar screen — one idea per step, as few as truly help (most answers 1, orientations 3-5, only genuinely complex screens 6-7).".to_string(),
